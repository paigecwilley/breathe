angular.module('medApp')
	.controller('regAndLoginCtrl', function($scope, $http, $state, userService){
		...
		$scope.createUser = function(){
			...
			$http.post('https://localhost:3000/create-user', JSON.stringify($scope.user)).then(function(successResponse){
		...
		$scope.loginUser = function(){
			...
			$http.post('https://localhost:3000/login', $scope.user).then(function(successResponse){
